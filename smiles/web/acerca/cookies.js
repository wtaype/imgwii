...
const COOKIES = [
  {
    ico: 'fa-shield-halved', color: '#0EBEFF',
    tit: 'Cookies Técnicas',
    desc: 'Esenciales para que la plataforma funcione. Permiten la navegación, el uso de herramientas de edición y el cambio de temas visuales.',
    necesaria: true
  },
  {
    ico: 'fa-chart-pie', color: '#FF5C69',
    tit: 'Análisis y Rendimiento',
    desc: 'Nos ayudan a entender cómo usas nuestras herramientas de imagen (cuáles son las más populares) para mejorar la velocidad y eficiencia.',
    necesaria: false
  },
  {
    ico: 'fa-user-gear', color: '#29C72E',
    tit: 'Personalización',
    desc: 'Recuerdan tus preferencias, como el último filtro usado o la configuración de compresión favorita, para que no tengas que reconfigurarlas.',
    necesaria: false
  }
];
...
<div class="ac_wrap ck_wrap">
  ...
  <section class="ac_hero ck_hero">
    ...
    <div class="ac_hero_body">
      ...
      <p class="ac_hero_sub">
        En <strong>${app}</strong> usamos cookies para mejorar tu experiencia de edición. 
        No usamos cookies intrusivas ni vendemos tus datos a terceros.
      </p>
      <div class="tm_hero_chips">
        <span class="tm_chip"><i class="fas fa-calendar-check"></i> Revisado: ${year()}</span>
    ...
    <div class="ck_grid">
      ${COOKIES.map(ck => `
        <div class="ck_card wi_fadeUp">
          <div class="ck_card_ico" style="background:color-mix(in srgb, ${ck.color} 15%, transparent); color: ${ck.color}">
            <i class="fas ${ck.ico}"></i>
          </div>
          <div class="ck_card_body">
            <h3>${ck.tit} ${ck.necesaria ? '<span class="ck_req">Obligatoria</span>' : ''}</h3>
            <p>${ck.desc}</p>
          </div>
        </div>
      `).join('')}
...
export const init = () => {
  ...
  console.log(`🍪 ${app} Cookies cargadas`);
};

export const cleanup = () => {
  console.log('🧹 Cookies limpiado');
};
